...
/* EVAL STEP, BEGIN! */
struct variable *lookup(const gchar * symbol, struct environment **envlist);

struct environment **copy_environment_list(struct environment **oldenv);

struct node *node_copy(struct node *oldnode);

struct node *read_list(void);

struct node *eval_if(struct node *expr, struct environment **env);

struct node *eval_cond(struct node *expr, struct environment **env);

struct node *eval_let(struct node *expr, struct environment **env);

struct node *eval_setcar(struct node *expr, struct environment **env);

struct node *eval_setcdr(struct node *expr, struct environment **env);

struct node *eval_define(struct node *expr, struct environment **env);

struct node *eval_lambda(struct node *expr, struct environment **env);

struct node *eval_delay(struct node *expr, struct environment **env);

struct node *eval_quote(struct node *expr, struct environment **env);

struct node *eval_application(struct node *expr, struct environment **env);

struct node *eval_load(struct node *expr, struct environment **env);

struct node *apply(struct node *proc, struct node **args, int n);

struct node *apply_compound(struct node *proc, struct node *args[], int n);

struct node *apply_prim(struct node *proc, struct node *args[], int n);
	...
	case PROC:{
			newnode =
			    procedure_to_node(oldnode->proc->symbols,
					      ...
					      copy_environment_list(oldnode->
								    proc->env));
			break;
		}
	case PAIR:{
			newnode = pair_to_node(node_copy(oldnode->pair->car),
					       node_copy(oldnode->pair->cdr));
			break;
		}
	case NIL:{
			newnode = nil_alloc();
...
void
bind_in_current_env(struct environment **envlist, const gchar * symbol,
		    struct node *value)
{
	int i, j;
...
struct node *lookup_value(struct environment **envlist, struct node *expr)
{
...
// evaluation
int special_form(struct node *expr)
{
	if (expr->type == SYMBOL) {
		if (expr->symbol == ("if")) {
			return IF;
...
struct node *eval_cond(struct node *expr, struct environment **env)
{
	...
	for (i = 1; i < expr->nlist; i++) {
		...
		if ((expr->list[i]->list[0]->type == SYMBOL
		     && (expr->list[i]->list[0]->symbol == ("else")))
...
gboolean istrue(struct node * expr)
{
...
int node_equal(struct node *n1, struct node *n2)
{
...
struct node *list_to_ll(struct node **nodelist, int n)
{
